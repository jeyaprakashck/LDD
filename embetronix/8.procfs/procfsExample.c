...
/************************************************************
 	CODE    : procfsExample.c
	Author  : JP
	Details : Real device driver with procFS
************************************************************/
...
static struct proc_dir_entry *path;

char *kernelBuf;
int value = 0;
char arr[1024] = "Hi User\n";
...
static int	helloProcOpen(struct inode *inode, struct file *file);
static int 	helloProcRelease(struct inode *inode, struct file *file);
static ssize_t 	helloProcRead(struct file *file, char __user *buf, size_t len, loff_t *offset);
static ssize_t	helloProcWrite(struct file *file, const char __user *buf, size_t len, loff_t *offset);
static long 	helloProcIOCTL(struct file *file, unsigned int cmd, unsigned long arg);
...
static struct file_operations procOps = {
	.open		= helloProcOpen,
	.read		= helloProcRead,
	.write		= helloProcWrite,
	.release 	= helloProcRelease,
};
...
/*
 * This function will be called whenever we open the proc file
 */
static int helloProcOpen(struct inode *inode, struct file *file){
	pr_info("Proc open function is called\n");
	return 0;
}

/*
 * This function will be called whenever we close the proc file
 */
static int  helloProcRelease(struct inode *inode, struct file *file){
	pr_info("proc close function is called\n");
	return 0;
}

/*
 * This function will be called whenever we write on proc file
 */
static ssize_t helloProcWrite(struct file *file, const char __user *buf, size_t len, loff_t *offset){
	pr_info("Proc write function is called\n");
	if(copy_from_user(kernelBuf, buf, len))
		pr_err("Error while writing..\n");
	pr_info("Data written successfully\n");
	return len;
...
/*
 * This function will be called whenever we read from the proc file
 */
static ssize_t helloProcRead(struct file *file, char __user *buf, size_t len, loff_t *offset){
       pr_info("Proc read function is called \n");
      //if(copy_to_user(buf, kernelBuf, len))
      //       pr_err("Error while reading\n");
       strcpy(buf, kernelBuf);
       pr_info("Data read successfully\n");
       return len;
}
...
static int __init helloWorldInit(void){
	...
	strcpy(kernelBuf, "Hello there\n");

	/* Creating proc directory */
	path = proc_mkdir("hello", NULL);
	if(path == NULL){
		pr_info("Error in creating proc entry");
	}

	/*creating proc entry under "/proc/hello/" */
	proc_create("helloProc", 0666, path, &procOps);
