...
/************************************************************
 	CODE    : gpioExamp;e.c
	Author  : JP
	Details : LED operations (On/Off/Toggle) using UART 
************************************************************/
...
//Serdev parameters
static int serdevProbe(struct serdev_device *serdev);
static void serdevRemove(struct serdev_device *serdev);
static int serdevReceiveFunc(struct serdev_device *serdev, const unsigned char *buf, size_t len);
static struct of_device_id serdevIDs[] = {
	{
		.compatible = "serDev",
	},
	{
...
MODULE_DEVICE_TABLE(of, serdevIDs);
static struct serdev_device_driver serdevDriver = {
	.probe = serdevProbe,
	.remove = serdevRemove,
	.driver = {
		.name = "serdev",
		.of_match_table = serdevIDs,
	},
};
static const struct serdev_device_ops serdevOps = {
	.receive_buf = serdevReceiveFunc,
};
static int serdevProbe(struct serdev_device *serdev){
	int status;
	printk("Inside the probe function \n");
	serdev_device_set_client_ops(serdev, &serdevOps);
	status = serdev_device_open(serdev);
	if(status){
		printk("Error in opening serial port\n");
		return -1;
	}
	serdev_device_set_baudrate(serdev, 9600);
	serdev_device_set_flow_control(serdev, false);
	serdev_device_set_parity(serdev, SERDEV_PARITY_NONE);
	printk("Probe function is executed\n");
	return 0;
}
static void serdevRemove(struct serdev_device *serdev){
	printk("Inside the remove function\n");
	serdev_device_close(serdev);
}
static int serdevReceiveFunc(struct serdev_device *serdev, const unsigned char *buf, size_t len){
	printk("Received char from UART\n");
	return 0;
}
...
static int __init helloWorldInit(void){
	...
	//Serial dev initialization
	if(serdev_device_driver_register(&serdevDriver)){
		printk("Error could not load serial driver\n");
	}
	/*
...
static void __exit helloWorldExit(void){
	serdev_device_driver_unregister(&serdevDriver);
