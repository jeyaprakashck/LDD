...
/************************************************************
 	CODE    : waitQueue.c
	Author  : JP
	Details : Real device driver with wait queue operations
************************************************************/
...
//wait queue variables
wait_queue_head_t waitQueue;
int waitQueueFlag = 0;
int readCount = 0;

//Kernel threads variables and functions
static struct task_struct *waitQueueThread;
static int waitQueueThreadFunc(void *data){
	while(1){
		printk("inside the thread... waiting for the event\n");
		wait_event_interruptible(waitQueue, waitQueueFlag == 1);
		if(waitQueueFlag ==2){
			printk("Event from exit function\n");
			return 0;
		}
		printk("Event came frokm read function and read count is %d\n", ++readCount);
		waitQueueFlag = 0;
	}
	return 0;
}
...
static int __init helloWorldInit(void){
	...
	/*thread creation */
	waitQueueThread = kthread_create(waitQueueThreadFunc, NULL, "waitThread");
	wake_up_process(waitQueueThread);
