...
/************************************************************
 	CODE    : tasklet.c
	Author  : JP
	Details : Real device driver with tasklet(static dynamic) operations
************************************************************/
...
void taskletFunc(unsigned long arg){
	printk("Inside the tasklet function: arg = %d \n", arg);
}
...
static int __init helloWorldInit(void){
	...
	tasklet_init(tasklet, taskletFunc, 10);
