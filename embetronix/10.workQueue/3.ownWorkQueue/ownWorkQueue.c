...
/************************************************************
 	CODE    : workQueue.c
	Author  : JP
	Details : Real device driver with own work queue(static method) operations
************************************************************/
...
//work queue variables3
static struct workqueue_struct *ownWorkQueueStruct;
static void ownWorkQueueFunc(struct work_struct *work){
	printk("Inside the workQueue function \n");
}
static DECLARE_WORK(ownWorkQueue, ownWorkQueueFunc);
...
static int __init helloWorldInit(void){
	...
	/* own work queue initialisation in static method */
	ownWorkQueueStruct = create_workqueue("ownWQ");
	queue_work(ownWorkQueueStruct, &ownWorkQueue);
...
static void __exit helloWorldExit(void){
	kfree(kernelBuf);
	destroy_workqueue(ownWorkQueueStruct);
