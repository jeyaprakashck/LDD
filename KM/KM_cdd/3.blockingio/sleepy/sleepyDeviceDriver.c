...
static ssize_t readFunc(struct file *filp, char __user *buf, size_t len, loff_t *off){
	...
	if(dataPresent == 0){
		if(wait_event_interruptible(myQueue, (dataPresent == 1)))
			printk("Signal occurs\n");
		else
			printk("Process awaken - Data ia available \n");
	}
	else
...
static int helloInit(void){
	...
	alloc_chrdev_region(&dev, 1, 1, "sleepyExample");
	...
	devClass = class_create(THIS_MODULE, "sleepyDeviceClass");
	
	/* creating device file */
	device_create(devClass, NULL, dev, NULL, "sleepyDeviceDriver");
