...
//workqueue parameters (dynamic method)
struct work_struct workQueue;
void workQueueFunc(struct work_struct *workQueue);	//bottom half function
...
void workQueueFunc(struct work_struct *workQueue){
	printk("Inside the workQueue function(BottomHalf)\n");
...
static ssize_t readFunc(struct file *file, char __user *buf, size_t count,loff_t *offset){
	...
	waitQueueFlag = 0;
	wait_event(waitQueue, waitQueueFlag == 1);
	waitQueueFlag = 0;
	return 0;
}
...
MODULE_DESCRIPTION("Keyboard handler example with workQueue");
